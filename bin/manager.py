...
class Manager(object):
    ...
    def __init__(self, config):
        ...
        try:
            master = self._config.find("master").get("value")
            if master == "1":
                self.isMaster = True
        except:
            pass

        log.info("SCI Started")

    def loadPlugins(self):
        elemPlugins = self._config.find('plugins')
        #load the plugins, if sci has been reloaded
    ...
    def _destoryCallback(self, request, plugin):
        plugin = plugin.lower()
